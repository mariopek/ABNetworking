...
class URLSessionHTTPClientTests: XCTestCase {
    ...
    func test_getFromURL_performsGETRequestWithURL() {
        ...
        makeSUT().request(from: request) { _ in }
    ...
    private func makeSUT(file: StaticString = #filePath, line: UInt = #line) -> HTTPClient {
        ...
        let sut = URLSessionHTTPClient(sessionConfig: configuration)
    ...
    private func resultFor(_ values: (data: Data?, response: URLResponse?, error: Error?)?, taskHandler: (HTTPClientTask) -> Void = { _ in },  file: StaticString = #filePath, line: UInt = #line) -> HTTPClient.Result {
        ...
        taskHandler(sut.request(from: anyURLRequest()) { result in
    ...
    private func anyHTTPURLResponse() -> HTTPURLResponse {
        return HTTPURLResponse(url: anyURLRequest().url!, statusCode: 200, httpVersion: nil, headerFields: nil)!
    }
    
    private func nonHTTPURLResponse() -> URLResponse {
        return URLResponse(url: anyURLRequest().url!, mimeType: nil, expectedContentLength: 0, textEncodingName: nil)
    }
